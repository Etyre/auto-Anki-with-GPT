...
const anki_questions_custom_functions = [
  {
    ...
    description:
      "Synthasize a series of useful AP test style question and answer from the body of the input text",
    parameters: {
      type: "object",
      properties: {
        q_1: {
...
const anki_cloze_custom_functions = [
  {
    ...
    description:
      "extract a key sentence from this paragraph. Something an AP test would find important. Create an anki style cloze {deletion} from that sentence where the small important part of the sentence is within single curly brackets.",
    parameters: {
      type: "object",
      properties: {
        key_sentence_1: {
        ...
        cloze_1: {
          type: "string",
          description:
            "the anki style {cloze deletion} based on key sentence 1",
        },
      },
    },
  },
...
//The function itself:
export async function propperIntializeOpenAIAPI(
  { extensionAPI },
  completedPrompt
) {
  const openai = new OpenAI({
